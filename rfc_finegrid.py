'''
Here, we train a random forest classifer on a fine grid of hyperparameters to optimise the chosen metrics, 
for predicting the class of toxicity of the organic ligand - the expectation here is to get the best performing model given the data.

The users can manually alter the hyperparameters passed to the model as well - however it is recommended they do so on the coarse-grid code. 
'''
...
def randomforestclassifier(X, y): 
    ...
    Users can tweak these, to test effects on model performance.

    The parameters are chosen with respect to the best performing model in the coarse grid. The expectation here is to get marginal improvements at best.

    '''
    param_grid = {'n_estimators' : [650, 675, 700, 725, 750],
                  'max_depth' : [32, 34, 36, 38, 40, 42, 44, 46, 48, 50]}

    model = RandomForestClassifier(n_jobs = -1, random_state = 0, min_samples_split = 2, min_samples_leaf = 1, verbose = 1)

    '''
    We perform a 5-fold cross-validation, with a single repeat (advised for large datasets). 

    '''
    cv = RepeatedStratifiedKFold(n_splits = 5, n_repeats = 1, random_state = 0)
    cv.get_n_splits(X, y)
    print('Starting randomized search ...')
    grid_search = GridSearchCV(estimator = model, param_grid = param_grid, scoring = scoring, cv = cv, refit = 'f1_macros')
    grid_search.fit(X, y)
...
def data(path):
    data_featurized = pd.read_csv(path) # change accordingly
    ...
    '''
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.2, random_state = 0) # these can be changed; in principle, we do not need to split the data here. 
    # the split is still maintained in-case we want to independently test the results.
