...
def roc_curve(classes, y_test, y_proba):
    '''
    ROC curves are typically used in binary classification, where TPR and FPR can be defined unambiguously.
    Since we are dealing with a multi-class classification - we need to binarize the outputs.

    We therefore follow a one-vs-rest (OvR) scheme. Each class against all others.
    '''
    ...
    for i in range(len(classes)):
        c = classes[i] # -1: fatal, 0: toxic and 1: safe
        cat = []
...
def plot(tpr, fpr, cat, auc):
   ...
   ax = plt.axes()
   ax.scatter(fpr, tpr, s = 0.4, color = 'red', linewidth = 0.4)
...
path = './data/oral_data_features_selected_sampled.csv' # add your filepath here.
path_test = './data/oral_test_set.csv'
...
filename = 'oral_finalized_model.sav'
