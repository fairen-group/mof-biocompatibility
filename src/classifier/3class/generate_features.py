###############################################################
#
# This script is for featurization of individual molecules when pass through the SMILES representation.
# In total, we calculate 197 descriptors - divided into different types based on the information of the molecule they capture.
# This script comes into use during the training process and later during training as well.
#
###############################################################
...
'''
In order to understand the physical meaning of these descriptors, please refer to the supporing information.
The first set of descriptors we will calculate are EState (Electro-topological State) descriptors. 
'''

def EState_Desc(smiles):
    m = Chem.MolFromSmiles(smiles) # extract a molecule object from a passed SMILES string.
...
def GeoDesc(smiles):
    '''
    The descriptions given here are all from Rdkit documentation.
    '''
    m = Chem.MolFromSmiles(smiles)
...
def MolSurf(smiles):
    '''
    Descriptions taken from Rdkit documentation.
    '''
    m = Chem.MolFromSmiles(smiles)
